...
The model for 3D printing was created using Autodesk Inventor with free Education Plan.  The equations that decribe the RF horn, Eqs. \ref{eq:1} and \ref{eq:2}, can be implemented with the parameters in Tab. \ref{tab:1} using the Equation Curve feature.  Inventor uses parametric modeling, meaning parameters from Tab. \ref{tab:1} can be edited to produce new designs.  Merging the mathematical form of Eqs. \ref{eq:1} and \ref{eq:2} required parameterization into a format useable by the Equation Curve feature.  Other challenges that are unique to this process involve maintaining geometric design constraints.  Since adjusting parameters or design offsets may result in a design that violates a design constraint, the process required fine tuning.
